...
// WiFi Hotspot (AP Mode) - for GBA Link direct connection
// Start WiFi hotspot on wlan1 with specified SSID and password
int PLAT_startHotspot(const char* ssid, const char* password);
// Stop WiFi hotspot
int PLAT_stopHotspot(void);
// Check if hotspot is running
bool PLAT_isHotspotActive(void);
// Get hotspot IP address (for display to user)
...
// Scan for GBALink hotspots (returns SSIDs starting with prefix)
int PLAT_wifiScanForGBALinkHotspots(char ssids[][33], int max_count);
// Scan for hotspots with a given prefix (generic version)
int PLAT_wifiScanForHotspots(const char* prefix, char ssids[][33], int max_count);
// Check if specific SSID is available in scan results
bool PLAT_wifiSSIDAvailable(const char* ssid);
// Connect to specific SSID with password (saves previous connection for restore)
int PLAT_wifiConnectToSSID(const char* ssid, const char* password);
// Restore previous WiFi connection after disconnecting from hotspot
